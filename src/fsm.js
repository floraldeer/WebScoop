    ...
    services: {
      invoke_初始化信息: () => (send) => {
        electronAPI
          ...
          .catch((err) => {
            // openssl 生成失败 / 权限异常等：明确落到未初始化，避免卡在 loading。
      ...
      invoke_开始初始化: () => (send) => {
        electronAPI
          ...
          .then((result) => {
            ...
            if (status === 'trusted') {
              send('e_重新检测');
            } else if (status === 'installed_untrusted') {
              message.warning(
          ...
          .catch((err) => {
            message.error(err?.message || '证书安装失败，请重试');
            send('e_初始化失败');
          });
      },
      invoke_启动服务: () => (send) => {
        ...
        electronAPI
          ...
          .then(() => {
            if (stopped) return;
            unsubscribe = electronAPI.on('VIDEO_CAPTURE', (data) =>
              fnDealVideoCapture('VIDEO_CAPTURE', data),
            );
        ...
        (send) => {
          ...
          electronAPI
            ...
            .catch((e) => {
              ...
              if (completed) return;
              completed = true;
              const canceled = /已取消下载/.test(String(e?.message || ''));
              send({ type: 'e_下载失败', canceled });
            });

          const onProgress = (arg) => {
            send({ type: 'e_进度变化', data: arg });
          };
          const unsubscribe = electronAPI.on('e_进度变化', onProgress);
    ...
    actions: {
      action_视频捕获: actions.assign(
        (
          ...
          const capturedAt = `${now.getFullYear()}-${pad(now.getMonth() + 1)}-${pad(
            now.getDate(),
          ...
          const isGenericWechatTitle = (title = '') =>
            /^(微信视频号视频|视频号视频|网络视频|未命名视频)$/.test(String(title).trim());
          const existingIndex = captureList.findIndex((item) => {
            ...
            if (item.shareUrl && newItem.shareUrl && item.shareUrl === newItem.shareUrl)
              return true;
            if (
              item.infoOnly &&
              newItem.uploader &&
              item.uploader &&
              item.uploader === newItem.uploader &&
          ...
          if (existingIndex >= 0) {
            ...
            updated[existingIndex] = {
              ...
              prettySize: newItem.size
                ? prettyBytes(+newItem.size)
                : gainedRealUrl
                ? '未知'
                : existing.prettySize,
            ...
            if (gainedRealUrl)
              message.success(`视频号真链已捕获: ${updated[existingIndex].description}`);
      ...
      action_下载失败: actions.assign((_, event) => {
        if (event?.canceled) {
          message.info('已取消下载');
        } else {
